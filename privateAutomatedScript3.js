...
const CONCEPTION_SCOPE_RETRY_DELAYS_MS = Object.freeze([0, 50, 150, 400]);
...
const samePendingSource = (candidate, eventData) => {
    ...
    if (!expectedKeys.length || (candidateRoot.World?.Time || '') !== (expectedRoot.World?.Time || '')) return false;
...
const findLatestMvuScope = runtime => {
...
const findUpdatedMvuScope = (runtime, variables) => {
    const matches = [];
    for (const messageId of candidateMessageIds(runtime)) {
        try {
            const data = getVariables({ type: 'message', message_id: messageId });
            if (samePendingSource(data, variables)) matches.push({ messageId, data });
    ...
    const preferred = Number.isInteger(runtime.lastAssistantMessageId)
        ...
        : null;
    if (preferred) return { scope: preferred, matchCount: matches.length };
    return { scope: matches.length === 1 ? matches[0] : null, matchCount: matches.length };
};
...
const buildPrivateUpdate = (sourceData, { resolveConception }) => {
    ...
    if (root.Date?.PhysioTick !== worldTime) { payload.Date = { PhysioTick: worldTime }; dirty = true; }
...
const applyPrivateUpdate = async (messageId, sourceData, options) => {
    ...
    if (payload) await insertOrAssignVariables(payload, { type: 'message', message_id: messageId });
    for (const path of cleanupPaths) await deleteVariable(path, { type: 'message', message_id: messageId });
};
...
const physioTick = runtime => enqueue(runtime, async () => {
    const scope = findLatestMvuScope(runtime);
    if (scope) await applyPrivateUpdate(scope.messageId, scope.data, { resolveConception: true });
});
const resolveAfterMvuUpdate = (runtime, variables) => {
    if (!pendingCheckKeys(variables).length) return;
    return enqueue(runtime, async () => {
        let lastMatchCount = 0;
        for (const delayMs of CONCEPTION_SCOPE_RETRY_DELAYS_MS) {
            await waitForRetry(delayMs);
            const result = findUpdatedMvuScope(runtime, variables);
            lastMatchCount = result.matchCount;
            if (!result.scope) continue;
            await applyPrivateUpdate(result.scope.messageId, result.scope.data, { resolveConception: true });
            return;
        }
        console.warn(`[DestinyPoetry] Conception resolver deferred after bounded retries: expected one updated MVU floor, found ${lastMatchCount}.`);
    });
};
...
const initPrivateAutomation = async () => {
    ...
    const runtime = { queue: Promise.resolve(), lastAssistantMessageId: null, subscriptions: [] };
    ...
    runtime.subscriptions.push(eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, variables => resolveAfterMvuUpdate(runtime, variables)));
