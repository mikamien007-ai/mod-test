...
const findUpdatedMvuScope = (runtime, variables) => {
    ...
    if (matches.length !== 1) {
        console.warn(`[DestinyPoetry] Conception resolver deferred: expected one updated MVU floor, found ${matches.length}.`);
        return null;
    }
    return matches[0];
};
...
const physioTick = runtime => enqueue(runtime, async () => {
    ...
    if (scope) await applyPrivateUpdate(scope.messageId, scope.data, { resolveConception: false });
});
const resolveAfterMvuUpdate = (runtime, variables) => {
    if (!pendingCheckKeys(variables).length) return;
    return enqueue(runtime, async () => {
        const scope = findUpdatedMvuScope(runtime, variables);
