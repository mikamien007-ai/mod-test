...
const pendingCheckKeys = data => {
    ...
    _.forEach(statRoot(data)?.RelationshipList || {}, (character, characterKey) => {
        const physiology = character?.PrivateProfile?.Physiology || {};
        const reproductive = character?.PrivateProfile?.ReproductiveStatus || {};
        const cycleLength = Number(physiology.CycleLengthDays) > 0 ? Math.round(Number(physiology.CycleLengthDays)) : 0;
        const modifierValue = Number(reproductive.ConceptionRateModifierPercent ?? 100);
        const modifier = Number.isFinite(modifierValue) ? _.clamp(Math.round(modifierValue), 0, 300) : 100;
        _.forEach(reproductive.ConceptionChecks || {}, (check, eventKey) => {
            if (!isPendingCheck(check)) return;
            keys.push([
                characterKey, eventKey, Number(check?.Sequence) || 0, daySignature(check?.OccurredAt),
                check?.ProtectionState || 'Unknown', cycleLength, daySignature(physiology.LastStartTime),
                Boolean(reproductive.IsPregnant), modifier,
            ].join('\u0000'));
        });
    });
    return keys.sort();
...
const privateSexualHistoryPathPattern = /^\/RelationshipList\/([^/]+)\/SexualHistory(?:\/(.*))?$/;
...
const normalizePrivateSexualHistoryCommands = (variables, commands) => {
    ...
    for (let index = commands.length - 1; index >= 0; index -= 1) {
        ...
        const match = privateSexualHistoryPathPattern.exec(sourcePath);
        const normalizedPath = match
            ? `/RelationshipList/${match[1]}/PrivateProfile/SexualHistory${match[2] ? `/${match[2]}` : ''}`
            : sourcePath;
        if (!normalizedPath.includes('/PrivateProfile/SexualHistory')) continue;
    ...
    return enqueue(runtime, async () => {
        ...
        for (const delayMs of CONCEPTION_SCOPE_RETRY_DELAYS_MS) {
            ...
            if (applied) {
                ...
                runtime.pendingMvuVariables = null;
            }
            return;
        }
        runtime.needsReconciliation = true;
        runtime.pendingMvuVariables = variables;
...
const initPrivateAutomation = async () => {
    ...
    const runtime = { queue: Promise.resolve(), lastAssistantMessageId: null, needsReconciliation: true, pendingMvuVariables: null, subscriptions: [] };
    ...
    runtime.subscriptions.push(eventOn(tavern_events.MESSAGE_UPDATED, messageId => {
        if (runtime.pendingMvuVariables) syncAfterMvuUpdate(runtime, runtime.pendingMvuVariables);
    ...
    const commandParsedSubscription = (typeof eventMakeFirst === 'function' ? eventMakeFirst : eventOn)(
        ...
        (variables, commands) => normalizePrivateSexualHistoryCommands(variables, commands),
    );
