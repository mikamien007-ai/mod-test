...
const relationshipSchema = zod.record(zod.string(), zod.object({
    ...
    Personality: zod.string().prefault(''), Likes: zod.string().prefault(''), Appearance: zod.string().prefault(''), Outfit: zod.any().transform(value => {
        const str = v => typeof v === 'string' ? v : '';
        if (typeof value === 'string') return { Overview: value, Top: '', Bottom: '', Underwear: '', ShoesSocks: '', Jewelry: '' };
        if (value && typeof value === 'object' && !Array.isArray(value)) return { Overview: str(value.Overview), Top: str(value.Top), Bottom: str(value.Bottom), Underwear: str(value.Underwear), ShoesSocks: str(value.ShoesSocks), Jewelry: str(value.Jewelry) };
        return { Overview: '', Top: '', Bottom: '', Underwear: '', ShoesSocks: '', Jewelry: '' };
    }),
