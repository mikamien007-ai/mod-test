...
const nonNegativeInteger = zod.coerce.number().prefault(0).transform(value => Math.max(0, Math.round(Number(value) || 0)));
...
const privatePhysiologySchema = zod.object({
    ...
    CycleState: zod.enum(['Active', 'SuspendedPregnancy', 'PostpartumAmenorrhea']).prefault('Active'),
    ...
    PostpartumStartTime: zod.string().prefault(''), CycleResumeTime: zod.string().prefault(''),
    CycleReturnConfirmedAt: zod.string().prefault(''),
    Symptoms: zod.array(zod.string()).prefault([]).transform(value => _.uniq(value)),
}).prefault({});
const ovulationRecordSchema = zod.object({
    CycleStartTime: zod.string().prefault(''),
    OvulationTime: zod.string().prefault(''),
    EggCount: zod.coerce.number().prefault(1).transform(value => _.clamp(Math.round(Number(value) || 1), 1, MAX_OVULATION_EGGS)),
}).prefault({});
const conceptionCheckSchema = zod.object({
    ...
    OvulationRecordKey: zod.string().prefault(''),
    OvulatedEggCount: nonNegativeInteger, FertilizedEggCount: nonNegativeInteger,
    AdditionalEggDiceRolls: zod.array(clampNumber(0, 1, 100)).prefault([]),
    Factors: zod.array(zod.string()).prefault([]).transform(value => _.uniq(value)),
    SkipReason: zod.string().prefault(''),
}).prefault({}).transform(value => {
    const ovulatedEggCount = _.clamp(value.OvulatedEggCount, 0, MAX_OVULATION_EGGS);
    return {
        ...value,
        OvulatedEggCount: ovulatedEggCount,
        FertilizedEggCount: _.clamp(value.FertilizedEggCount, 0, ovulatedEggCount),
        AdditionalEggDiceRolls: value.AdditionalEggDiceRolls.slice(0, Math.max(0, ovulatedEggCount - 1)),
    };
});
const privateEventFactSchema = zod.object({
    Type: zod.literal('InternalEjaculation'),
    PartnerCharacterKey: zod.string().prefault(''),
    ProtectionState: zod.enum(['None', 'Failed', 'Effective', 'Unknown']).prefault('Unknown'),
}).prefault({});
const privateReproductiveSchema = zod.object({
    ...
    OvulationRecords: zod.record(zod.string(), ovulationRecordSchema).prefault({}),
...
const derivePrivateReproductiveStatus = (physiology, reproductiveStatus) => {
    ...
    const cycleState = physiology?.CycleState || 'Active';
    ...
    } else if (cycleState === 'PostpartumAmenorrhea') {
        derived = { ...reproductive, Fertility: 'Chưa trở lại sau thai kỳ', OvulationStatus: 'Chưa trở lại sau thai kỳ', FertilizableEggCount: 0 };
    } else if (cycleState === 'Active' && cycleLength > 0) {
        const isOvulating = physiology?.CurrentPhase === 'Rụng trứng';
        const eggCount = isOvulating ? _.clamp(Math.round(Number(reproductive.FertilizableEggCount) || 0), 0, MAX_OVULATION_EGGS) : 0;
        derived = { ...reproductive, Fertility: isOvulating ? 'Cao' : 'Bình thường', OvulationStatus: isOvulating ? 'Đang rụng trứng' : 'Không rụng trứng', FertilizableEggCount: eggCount };
    }
    return {
        ...derived,
...
const privateProfileSchema = zod.object({
    ...
    EventFacts: zod.record(zod.string(), privateEventFactSchema).prefault({}),
